...
export default class GoogleLogin extends Component {
  ...
  handleCallback = () => {
    ...
    const redirectUrl = encodeURIComponent('http://localhost:3000'); // Redirect Url -> handleLogin의 redirectUri와 반드시 일치
    ...
    if (authorizationCode) {
      axios
        ...
        .then((response) => {
          ...
          useNavigate('/index', { isLogin: true})  // 로그인확인 후 메인 페이지로 이동
        })
        .catch((error) => {
          if (error.response.status === 401) { // DB에 사용저 정보 X -> 회원가입 페이지로 이동
            
            useNavigate('/SignUp', { name: error.response.name, email: error.response.email}) // 정보 전달하면서 회원가입 페이지로 이동
          }
        });
    }
  };
